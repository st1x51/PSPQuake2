...
*/
float part_scale = 1.0f / DS_GEOM_SCALE;
...
// transform point
	VectorSubtract (pparticle->origin, r_origin, local);
	local_fp[0] = (int)(local[0] * 8192);
	local_fp[1] = (int)(local[1] * 8192);
	local_fp[2] = (int)(local[2] * 8192);

	bind_texture(-1);

	DS_COLOUR3B(sw_state.currentpalette[p->color * 4],
			sw_state.currentpalette[p->color * 4 + 1],
			sw_state.currentpalette[p->color * 4 + 2]);
	
	DS_BEGIN_QUAD();
	
	DS_VERTEX3V16(local_fp[0] / (2 * DS_GEOM_SCALE),
			local_fp[1] / (2 * DS_GEOM_SCALE),
			local_fp[2] / (2 * DS_GEOM_SCALE));
	DS_VERTEX3V16((local_fp[0] + part_vup[0] * 2) / (2 * DS_GEOM_SCALE),
			(local_fp[1] + part_vup[1] * 2) / (2 * DS_GEOM_SCALE),
			(local_fp[2] + part_vup[2] * 2) / (2 * DS_GEOM_SCALE));
	DS_VERTEX3V16((local_fp[0] + part_vup[0] * 2 + part_vright[0] * 2) / (2 * DS_GEOM_SCALE),
			(local_fp[1] + part_vup[1] * 2 + part_vright[1] * 2) / (2 * DS_GEOM_SCALE),
			(local_fp[2] + part_vup[2] * 2 + part_vright[2] * 2) / (2 * DS_GEOM_SCALE));
	DS_VERTEX3V16((local_fp[0] + part_vright[0] * 2) / (2 * DS_GEOM_SCALE),
			(local_fp[1] + part_vright[1] * 2) / (2 * DS_GEOM_SCALE),
			(local_fp[2] + part_vright[2] * 2) / (2 * DS_GEOM_SCALE));
}
...
extern int part_vright[3];
extern int part_vup[3];
extern int part_vpn[3];
...
//	VectorCopy( vpn, r_ppn );
	...
	{
		part_vright[count] = (int)(vright[count] * 8192);
		part_vup[count] = (int)(vup[count] * 8192);
		part_vpn[count] = (int)(vpn[count] * 8192);
	}
