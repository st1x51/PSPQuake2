...
int max_distance = 430;
int transparent_distance = 380;
float transparent_difference = 50;			//real hard to compute
short min_point[3];
short size_point[3];
bool transparency_on;

bool ds_buildbbox(daliasframe_t *frame)
{
	int i;
	vec3_t        mins, maxs;
	...
	*/
	...
	for (i = 0; i < 3; i++)
	{
		min_point[i] = (short)(transformed_min[i] * 16);
		size_point[i] = (short)((transformed_max[i] - transformed_min[i]) * 16);
	}
	
//	for (i = 0; i < 3; i++)
//	{
//		min_point[i] = (short)(mins[i] * 16);
//		size_point[i] = (short)((frame->scale[i]*255) * 16);
//	}
	
	transparency_on = false;
	
	if ( transformed_min[2] >= ALIAS_Z_CLIP_PLANE && transformed_min[2] < max_distance)
	{
		if (transformed_min[2] > transparent_distance)
		{
			transparency_on = true;
		
			float transparency = (transformed_min[2] - transparent_distance) / transparent_difference * 30.0f;
			ds_polyfmt(1, 31 - (int)transparency - 1, 0, POLY_CULL_FRONT);
		}
		return false;
	}
	if ( transformed_max[2] >= ALIAS_Z_CLIP_PLANE && transformed_max[2] < max_distance)
	{
		if (transformed_max[2] > transparent_distance)
		{
			transparency_on = true;
		
			float transparency = (transformed_max[2] - transparent_distance) / transparent_difference * 30.0f;
			ds_polyfmt(1, 31 - (int)transparency - 1, 0, POLY_CULL_FRONT);
		}
		return false;
	}
...
//						((CACHE_SIZE - 1) / sizeof(finalvert_t)) + 3];
	...
	if (s_pmdl->num_xyz > MAXALIASVERTS)
	{
		printf("model too large to render %d\n", s_pmdl->num_xyz);
		return;
	}
...
//
	...
	for (i=0 ; i<s_pmdl->num_tris ; i++, ptri++)
	{
		...
		DS_TEXCOORD2T16((short)( pstverts[ptri->index_st[0]].t << 3), (short)( pstverts[ptri->index_st[0]].s << 4));
		DS_VERTEX3V16((short)(pfv[0]->short_xyz[0]), (short)(pfv[0]->short_xyz[1]), (short)(pfv[0]->short_xyz[2]));
		
		DS_TEXCOORD2T16((short)( pstverts[ptri->index_st[1]].t << 3), (short)( pstverts[ptri->index_st[1]].s << 4));
		DS_VERTEX3V16((short)(pfv[1]->short_xyz[0]), (short)(pfv[1]->short_xyz[1]), (short)(pfv[1]->short_xyz[2]));
		
		DS_TEXCOORD2T16((short)( pstverts[ptri->index_st[2]].t << 3), (short)( pstverts[ptri->index_st[2]].s << 4));
		DS_VERTEX3V16((short)(pfv[2]->short_xyz[0]), (short)(pfv[2]->short_xyz[1]), (short)(pfv[2]->short_xyz[2]));
	}
...
void R_AliasTransformFinalVerts( int numpoints, finalvert_t *fv, dtrivertx_t *oldv, dtrivertx_t *newv )
{
	...
	int frame_scale[3];
	int frame_translate[3];
	
	for (i = 0; i < 3; i++)
	{
		frame_scale[i] = (int)(r_thisframe->scale[i] * 512);
		frame_translate[i] = (int)(r_thisframe->translate[i] * 512);
	}
...
//		lerped_vert[0] = r_thisframe->translate[0] + newv->v[0]*r_thisframe->scale[0];
//		lerped_vert[1] = r_thisframe->translate[1] + newv->v[1]*r_thisframe->scale[1];
//		lerped_vert[2] = r_thisframe->translate[2] + newv->v[2]*r_thisframe->scale[2];
...
//		fv->short_xyz[0] = lerped_vert[0] * 16;
//		fv->short_xyz[1] = lerped_vert[1] * 16;
//		fv->short_xyz[2] = lerped_vert[2] * 16;
		
		fv->short_xyz[0] = (frame_translate[0] + newv->v[0] * frame_scale[0]) >> 5;
		fv->short_xyz[1] = (frame_translate[1] + newv->v[1] * frame_scale[1]) >> 5;
		fv->short_xyz[2] = (frame_translate[2] + newv->v[2] * frame_scale[2]) >> 5;

		fv->flags = 0;
		
//		abort_render |= ds_boxtest(fv->short_xyz[0], fv->short_xyz[1], fv->short_xyz[2], 1, 1, 1);
...
*/
#if 0
void R_AliasSetupLightingEasy (char *model_colour)
{
	alight_t		lighting;
	...
	model_colour[0] = (char)(light[0] * 255);
	model_colour[1] = (char)(light[1] * 255);
	model_colour[2] = (char)(light[2] * 255);
}
#else
void R_AliasSetupLighting (void)
{
	...
	j = (int)((light[0] + light[1] + light[2])*0.3333f*255);
...
float alias_x_scale = 1.0f;
float alias_y_scale = 1.0f;
float alias_z_scale = 1.0f;

float alias_x_pos = 1.0f;
float alias_y_pos = 1.0f;
float alias_z_pos = 1.0f;

float alias_x_add = 0.0f;
...
//	}
	...
	if ( !R_AliasSetupSkin () )
	{
		...
		if (transparency_on)
			ds_polyfmt(0, 31, 0, POLY_CULL_FRONT);
...
//	}
	...
	*/
	...
	ds_translatef(aliastransform[0][3] / 256, - aliastransform[1][3] / 256, -aliastransform[2][3] / 256);
	
//	aliastransform[0][3] = 0;
//	aliastransform[1][3] = 0;
//	aliastransform[2][3] = 0;
	
	if ( ( currententity->flags & RF_WEAPONMODEL ) && ( r_lefthand->value == 1.0F ) )
	{
		alias_x_scale = -alias_x_scale;
		ds_polyfmt(0, 31, 0, POLY_CULL_BACK);
	}
	
	ds_scalef(1 * alias_x_scale, -1 * alias_y_scale, -1 * alias_z_scale);
	
//	printf("result is %d\n", ds_boxtest(min_point[0],
//						min_point[1],
//						min_point[2],
//						size_point[1],
//						size_point[0],
//						size_point[2]));
	...
	{
		drew_model++;
		bool bound = bind_texture(pskindesc->texture_handle);
		
		if (currententity->flags & RF_TRANSLUCENT)
		{
			ds_polyfmt(1, (int)(28.0f * currententity->alpha) + 1, 0, POLY_CULL_FRONT);
			transparency_on = true;
		}
		
		if ((transparency_on && bound) || !transparency_on)
		{
			ds_mulmatf(aliastransform);
			R_AliasPreparePoints ();
		}
	}
