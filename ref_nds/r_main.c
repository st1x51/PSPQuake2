...
short int	*r_pcurrentrealvertbase, *vertbase_cache = NULL;

//short int scaled_modelorg[3];
...
extern mtexinfo_t *last_texinfo;
...
void R_Register (void)
{
	...
	sw_surfcacheoverride = ri.Cvar_Get ("sw_surfcacheoverride", "100", 0);
	sw_waterwarp = ri.Cvar_Get ("sw_waterwarp", "0", 0);
	sw_mode = ri.Cvar_Get( "sw_mode", "1", CVAR_ARCHIVE );

	r_lefthand = ri.Cvar_Get( "hand", "0", CVAR_USERINFO | CVAR_ARCHIVE );
	r_speeds = ri.Cvar_Get ("r_speeds", "0", 0);
	r_fullbright = ri.Cvar_Get ("r_fullbright", "0", 0);
	r_drawentities = ri.Cvar_Get ("r_drawentities", "0", 0);
	...
	byte_write(&sw_mode->modified, true); //	sw_mode->modified = true; // force us to do mode specific stuff later
	byte_write(&vid_gamma->modified, true); //	vid_gamma->modified = true; // force us to rebuild the gamma table later
...
// TODO: collect 386-specific code in one place
	...
	ri.Con_Printf (PRINT_ALL, "ref_nds version: "REF_VERSION"\n");
...
////		R_SurfacePatch ();
//		
//		Sys_Error("%d surfs - we ain\'t gonna bother allocate this!\n", r_cnumsurfs);
//	}
//	else
//	{
//		r_surfsonstack = true;
...
void R_DrawBeamEntitiesOnList (void)
{
	int			i;
...
//		ds_rotateZ(currententity->angles[1]);
//		ds_rotateY(currententity->angles[0]);
//		ds_rotateX(currententity->angles[2]);

	// calculate dynamic lighting for bmodel
		R_PushDlights (currentmodel);
		
		ds_translatef(modelorg[0] / -DS_GEOM_SCALE, modelorg[1] / -DS_GEOM_SCALE, modelorg[2] / -DS_GEOM_SCALE);
		
		ds_rotateZ(currententity->angles[1]);
		ds_rotateY(currententity->angles[0]);
		ds_rotateX(currententity->angles[2]);
		
		R_ZDrawSubmodelPolys (currentmodel);
...
//	edge_t *ledges = (edge_t *)ds_malloc(NUMSTACKEDGES +
//				((CACHE_SIZE - 1) / sizeof(edge_t)) + 1);
//	surf_t *lsurfs = (surf_t *)ds_malloc(NUMSTACKSURFACES +
//				((CACHE_SIZE - 1) / sizeof(surf_t)) + 1);
//	ds_set_malloc_base(MEM_MAIN);

	if ( r_newrefdef.rdflags & RDF_NOWORLDMODEL )
	{
//		ds_free(ledges); ds_free(lsurfs);
		return;
	}
...
//	R_BeginEdgeFrame ();
	...
	ds_pushmatrix();
		...
		ds_scalef(1 << DS_SCALE_FUNK, 1 << DS_SCALE_FUNK, 1 << DS_SCALE_FUNK);
...
//	
		...
		ds_polyfmt(2, 10, 0, POLY_CULL_NONE);
		R_DrawBeamEntitiesOnList();
		
		ds_polyfmt(2, 20, 0, POLY_CULL_NONE);
		R_DrawParticles ();
...
void R_CalcPalette (void)
{
	...
	for (i=0 ; i<256 ; i++, in+=4, out+=4)
	{
		for (j=0 ; j<3 ; j++)
		{
			v = (int)(premult[j] + one_minus_alpha * in[j]);
			if (v > 255)
				v = 255;
			out[j] = v;
		}
...
extern unsigned int drew_model;
extern int drawn_polys, perp_sides, angled_sides, clipped_faces, drawn_tris;
...
//	unsigned int polys, verts;
//	ds_get_geometry_count(&verts, &polys);
//	printf("drew %d models, %d v %d p\n", drew_model, verts, polys);
//	drew_model = 0;
	
//	trace_t tr;
//	vec3_t	offset, end_pos;
//	
//	VectorSet(offset, 1000, 0, 0);
//	ProjectSource(r_refdef.vieworg, offset, vpn, vright, end_pos);
//	tr = CM_BoxTrace (r_newrefdef.vieworg, end_pos, vec3_origin, vec3_origin, 0, MASK_SOLID);
//	
//	if (tr.fraction < 1.0)
//	{
//		Com_Printf("%s\n", tr.surface->name);
...
//	printf("p %d a %d d %d\n", perp_sides, angled_sides, drawn_polys);
//	printf("clipped %d\n", clipped_faces);
...
//		vid_gamma->modified = false;
		byte_write(&vid_gamma->modified, false);
	}
	...
	while ( sw_mode->modified || vid_fullscreen->modified )
	{
		...
		*/
		if ( ( err = SWimp_SetMode( &vid.width, &vid.height, (int)(sw_mode->value), vid_fullscreen->value ) ) == rserr_ok )
		{
			R_InitGraphics( vid.width, vid.height );

			sw_state.prev_mode = (int)(sw_mode->value);
//			vid_fullscreen->modified = false;
			byte_write(&vid_fullscreen->modified, false);
//			sw_mode->modified = false;
			byte_write(&sw_mode->modified, false);
		}
		else
		{
...
#define NUM_BEAM_SEGS 6
	...
	VectorSubtract(origin, r_origin, origin);
	VectorSubtract(oldorigin, r_origin, oldorigin);
	...
	int r = ( d_8to24table[e->skinnum & 0xFF] ) & 0xFF;
	int g = ( d_8to24table[e->skinnum & 0xFF] >> 8 ) & 0xFF;
	int b = ( d_8to24table[e->skinnum & 0xFF] >> 16 ) & 0xFF;
...
//	DS_COLOUR3B(255, 255, 255);
	...
	DS_BEGIN_TRIANGLE_STRIP();
	
	for ( i = 0; i < NUM_BEAM_SEGS; i++ )
	{
		DS_VERTEX3V16( (int)(start_points[i][0] * DS_MINI_GEOM_SCALE),
			(int)(start_points[i][1] * DS_MINI_GEOM_SCALE),
			(int)(start_points[i][2] * DS_MINI_GEOM_SCALE) );
		DS_VERTEX3V16( (int)(end_points[i][0] * DS_MINI_GEOM_SCALE),
			(int)(end_points[i][1] * DS_MINI_GEOM_SCALE),
			(int)(end_points[i][2] * DS_MINI_GEOM_SCALE) );
		DS_VERTEX3V16( (int)(start_points[(i+1)%NUM_BEAM_SEGS][0] * DS_MINI_GEOM_SCALE),
			(int)(start_points[(i+1)%NUM_BEAM_SEGS][1] * DS_MINI_GEOM_SCALE),
			(int)(start_points[(i+1)%NUM_BEAM_SEGS][2] * DS_MINI_GEOM_SCALE) );
		DS_VERTEX3V16( (int)(end_points[(i+1)%NUM_BEAM_SEGS][0] * DS_MINI_GEOM_SCALE),
			(int)(end_points[(i+1)%NUM_BEAM_SEGS][1] * DS_MINI_GEOM_SCALE),
			(int)(end_points[(i+1)%NUM_BEAM_SEGS][2] * DS_MINI_GEOM_SCALE) );
	}
}
...
//		printf("sky texture %s at %08x, %d\n", pathname, r_skytexinfo[i].image, r_skytexinfo[i].image->texture_handle);
	}
	set_sky_not_loading();
