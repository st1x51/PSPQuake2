...
void R_PrintDSpeeds (void)
{
	...
	da_time = (int)(da_time2 - da_time1);
	dp_time = (int)(dp_time2 - dp_time1);
	rw_time = (int)(rw_time2 - rw_time1);
	db_time = (int)(db_time2 - db_time1);
	se_time = (int)(se_time2 - se_time1);
	de_time = (int)(de_time2 - de_time1);
	ms = (int)(r_time2 - r_time1);
...
void R_TransformFrustum (void)
{
	...
	for (i=0 ; i<4 ; i++)
	{
		...
		view_clipplanes[i].normal_int[0] = (int)(view_clipplanes[i].normal[0] * 1024);
		view_clipplanes[i].normal_int[1] = (int)(view_clipplanes[i].normal[1] * 1024);
		view_clipplanes[i].normal_int[2] = (int)(view_clipplanes[i].normal[2] * 1024);

		view_clipplanes[i].dist = DotProduct (modelorg, v2);
		
		view_clipplanes[i].dist_int = (int)(view_clipplanes[i].dist * 1024);
	}
}
...
{
	Sys_Error("pcx saving doesn\'t work\n");
//	int			i, j, length;
//	pcx_t		*pcx;
//	byte		*pack;
//	FILE		*f;
...
//	f = ds_fopen (filename, "wb");
//	if (!f)
//		ri.Con_Printf (PRINT_ALL, "Failed to open to %s\n", filename);
//	else
//	{
//		ds_fwrite ((void *)pcx, 1, length, f);
//		ds_fclose (f);
//	}
//
//	ds_free (pcx);
...
{
	Sys_Error("screenshots don\'t work\n");
//	int			i; 
//	char		pcxname[80]; 
