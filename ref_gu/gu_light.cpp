	...
	for (i=0 ; i<node->numsurfaces ; i++, surf++)
	{
		...
		if (lightmap)
		{
					...
					maps++)
			{
				for (i=0 ; i<3 ; i++)
				{
					scale[i] = gu_modulate->value*r_newrefdef.lightstyles[surf->styles[maps]].rgb[i];
				}
...
void R_LightPoint (vec3_t p, vec3_t color)
{
	...
	VectorScale (color, gu_modulate->value, color);
}
...
store:
	...
	monolightmap = gu_monolightmap->string[0];
