...
void Mod_FreeAll(void)
{
    ...
    memset(mod_novis,  0xff, sizeof(mod_novis  ));
	memset(mod_inline, 0x00, sizeof(mod_inline ));
	memset(mod_known,  0x00, sizeof(mod_known  ));
	mod_numknown = 0;
}
...
model_t* Mod_ForName(const char* name, qboolean crash) 
{
	...
	case IDBSPHEADER :
		//loadmodel->extradata = Hunk_Begin(9 * 1024 * 1024);
		loadmodel->extradata = Hunk_Begin(maphunk * 1024 * 1024);
...
void LightMapClear (void)
{
   ...
   for(i = 0 ; i < 128 ; i++)
   {
		...
		GL_UnloadTexture(lightmap_textures[i]->texnum);
	}
...
void Mod_LoadPlanes(lump_t *l) 
{
	...
	//memset(out, 0, count*2*sizeof(*out));
