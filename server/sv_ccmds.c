...
void CopyFile (char *src, char *dst)
{
	...
	{
#ifdef IOCTRL
		l = FS_Fread (buffer, 1, sizeof(buffer), f1);
#else
...
#ifdef IOCTRL
	FS_Fwrite (sv.configstrings, sizeof(sv.configstrings), 1, f);
#else
...
void SV_WriteServerFile (qboolean autosave)
{
...
#ifdef IOCTRL
	FS_Fwrite (comment, 1, sizeof(comment), f);
#else
	...
	// write the mapcmd
#ifdef IOCTRL
	FS_Fwrite (svs.mapcmd, 1, sizeof(svs.mapcmd), f);
#else
...
#ifdef IOCTRL
        FS_Fwrite (name, 1, sizeof(name), f);
		FS_Fwrite (string, 1, sizeof(string), f);
#else
...
#ifdef IOCTRL
	FS_Fwrite (&len, 4, 1, svs.demofile);
	FS_Fwrite (buf.data, buf.cursize, 1, svs.demofile);
#else
