...
{
	Cvar_SetValue("net_wificonnection", ClampCvar(0, 10, s_wifi_box.curvalue));
}
...
void Multiplayer_MenuInit(void) 
{
	...
	s_wifi_box.generic.type = MTYPE_SPINCONTROL;
	s_wifi_box.generic.x    = -55;
	s_wifi_box.generic.y	= 30;
	s_wifi_box.generic.name = "WiFi Point: ";
	s_wifi_box.generic.cursor_offset = -55;
	s_wifi_box.generic.callback = SelectWifiConnection;
	s_wifi_box.curvalue  = net_wificonnection->value;
	s_wifi_box.itemnames = wificonnection_items;

    if(sceWlanGetSwitchState())
	{
		Menu_AddItem( &s_multiplayer_menu, &s_wifi_box);
	}
...
static void Options_SetValues(void) 
{
	...
	extern cvar_t* in_analog_speed;
	Cvar_SetValue("in_analog_speed", ClampCvar(1.0f, 9.0f, in_analog_speed->value));
	s_options_speed.curvalue = in_analog_speed->value * 2.0f;

	extern cvar_t* in_analog_tolerance;
	Cvar_SetValue("in_analog_tolerance", ClampCvar(0.0f, 0.2f, in_analog_tolerance->value));
	s_options_tolerance.curvalue = in_analog_tolerance->value * 20.0f;

	extern cvar_t* in_analog_acceleration;
	Cvar_SetValue("in_analog_acceleration", ClampCvar(0.0f, 4.0f, in_analog_acceleration->value));
	s_options_acceleration.curvalue = in_analog_acceleration->value;

	extern cvar_t* in_analog_anubfunc;
	Cvar_SetValue("in_analog_anubfunc", ClampCvar(0.0f, 2.0f, in_analog_anubfunc->value));
	s_options_anubfunc.curvalue = in_analog_anubfunc->value;

	extern cvar_t* in_analog_anubinvert;
	Cvar_SetValue("in_analog_anubinvert", ClampCvar(0.0f, 1.0f, in_analog_anubinvert->value));
	s_options_anubinvert.curvalue = in_analog_anubinvert->value;
	...
	extern cvar_t* m_volume;
	Cvar_SetValue("m_volume", ClampCvar(0, 1, m_volume->value));
	s_options_musicvolume.curvalue = m_volume->value * 10.0f;
...
{
	Cvar_SetValue("in_analog_speed", s_options_speed.curvalue / 2.0f);
}

static void f_options_tolerance(void* unused)
{
	Cvar_SetValue("in_analog_tolerance", s_options_tolerance.curvalue / 20.0f);
}

static void f_options_acceleration(void* unused)
{
	Cvar_SetValue("in_analog_acceleration", s_options_acceleration.curvalue);
}

static void f_options_anubfunc(void* unused)
{
	Cvar_SetValue("in_analog_anubfunc", s_options_anubfunc.curvalue);
}

static void f_options_anubinvert(void *unused)
{
	Cvar_SetValue("in_analog_anubinvert", s_options_anubinvert.curvalue);
}
...
{
	Cvar_SetValue("m_volume", s_options_musicvolume.curvalue / 10.0f);
}
...
#ifdef SOFT_RENDER
static menulist_s		s_video_stipplealpha;
#else
static menulist_s		s_video_vsync;
static menulist_s		s_video_dynamic;
static menulist_s		s_video_particletype;
static menulist_s		s_video_tga_sky;
static menulist_s		s_video_mipmaps;
static menulist_s	    s_video_mipmaps_func;
static menulist_s	    s_video_lightmap_mode;
static menuslider_s	    s_video_mipmaps_bias;
#endif
...
{
#ifdef SOFT_RENDER
      cvar_t* sw_stipplealpha  = Cvar_Get("sw_stipplealpha",  "1", 0);
#elif GU_RENDER	
      cvar_t* gu_vsync         = Cvar_Get("gu_vsync",         "0", 0);
      cvar_t* gu_dynamic       = Cvar_Get("gu_dynamic",       "0", 0);
	  cvar_t* gu_particletype  = Cvar_Get("gu_particletype",  "0", 0);
	  cvar_t* gu_tgasky        = Cvar_Get("gu_tgasky",        "0", 0);
	  cvar_t* gu_mipmaps       = Cvar_Get("gu_mipmaps",       "0", 0);
	  cvar_t* gu_mipmaps_func  = Cvar_Get("gu_mipmaps_func",  "0", 0);
      cvar_t* gu_lightmap_mode = Cvar_Get("gu_lightmap_mode", "0", 0);
	  cvar_t* gu_mipmaps_bias  = Cvar_Get("gu_mipmaps_bias",  "0", 0);
#endif
#ifdef SOFT_RENDER
    sw_stipplealpha->value = ClampCvar(0, 1, sw_stipplealpha->value);
	s_video_stipplealpha.curvalue = sw_stipplealpha->value;
#elif GU_RENDER	
	gu_vsync->value = ClampCvar(0, 1, gu_vsync->value);
	s_video_vsync.curvalue = gu_vsync->value;

	gu_dynamic->value = ClampCvar(0, 1, gu_dynamic->value);
	s_video_dynamic.curvalue = gu_dynamic->value;

	gu_particletype->value = ClampCvar(0, 2, gu_particletype->value);
	s_video_particletype.curvalue = gu_particletype->value;

	gu_tgasky->value = ClampCvar(0, 2, gu_tgasky->value);
	s_video_tga_sky.curvalue = gu_tgasky->value;

	gu_mipmaps->value = ClampCvar(0, 1, gu_mipmaps->value);
	s_video_mipmaps.curvalue = gu_mipmaps->value;

	gu_mipmaps_func->value = ClampCvar(0, 2, gu_mipmaps_func->value);
	s_video_mipmaps_func.curvalue = gu_mipmaps_func->value;

	gu_mipmaps_bias->value = ClampCvar(-10, 0, gu_mipmaps_bias->value);
	s_video_mipmaps_bias.curvalue = gu_mipmaps_bias->value;

	gu_lightmap_mode->value = ClampCvar(0, 7, gu_lightmap_mode->value);
	s_video_lightmap_mode.curvalue = gu_lightmap_mode->value;
#endif
...
#ifdef SOFT_RENDER
static void f_video_stipplealpha(void* unused) 
{
	Cvar_SetValue("sw_stipplealpha", s_video_stipplealpha.curvalue);
}
#elif GU_RENDER	
...
static void f_video_dynamic(void* unused)
{
	Cvar_SetValue("gu_dynamic", s_video_dynamic.curvalue);
}
...
static void f_video_tga_sky(void* unused)
{
	Cvar_SetValue("gu_tgasky", s_video_tga_sky.curvalue);
}

static void f_video_mipmaps(void* unused)
{
	Cvar_SetValue("gu_mipmaps", s_video_mipmaps.curvalue);
}

static void f_video_mipmaps_func(void* unused)
{
	Cvar_SetValue("gu_mipmaps_func", s_video_mipmaps_func.curvalue);
}

static void f_video_mipmaps_bias(void* unused)
{
	Cvar_SetValue("gu_mipmaps_bias", s_video_mipmaps_bias.curvalue);
}

static void f_video_lightmap_mode(void* unused)
{
	Cvar_SetValue("gu_lightmap_mode", s_video_lightmap_mode.curvalue);
}
...
void VID_MenuInit(void) 
{
	...
	const static char* menu_mipmaps_func_change[] =
	{
      "auto", "const", "slope", 0
	};

	const static char* menu_lightmap_mode_change[] =
	{
        "PSM_5650 (16 bit)", "PSM_5551 (16 bit)", "PSM_4444 (16 bit)", "PSM_8888 (32 bit)", "PSM_DXT1 (32 bit compressed)", "PSM_DXT3 (32 bit compressed)", "PSM_DXT5 (32 bit compressed)", 0
	};
...
#ifdef SOFT_RENDER
	s_video_stipplealpha.generic.type				= MTYPE_SPINCONTROL;
	s_video_stipplealpha.generic.x					= 0;
	s_video_stipplealpha.generic.y					= offset;
	s_video_stipplealpha.generic.name				= "stipple alpha";
	s_video_stipplealpha.generic.callback			= f_video_stipplealpha;
	s_video_stipplealpha.itemnames					= menu_disabledenabled_names;

	offset += 30;
    extern cvar_t* vid_gamma;
    s_brightness_slider.generic.type	= MTYPE_SLIDER;
	s_brightness_slider.generic.x	= 0;
	s_brightness_slider.generic.y	= offset;
	...
	s_video_vsync.generic.y				    = offset;
	s_video_vsync.generic.name			    = "VSync";
	s_video_vsync.generic.callback		    = f_video_vsync;
	s_video_vsync.itemnames				    = menu_noyes_names;

	offset += 10;
	s_video_dynamic.generic.type			= MTYPE_SPINCONTROL;
	s_video_dynamic.generic.x				= 0;
	s_video_dynamic.generic.y				= offset;
	s_video_dynamic.generic.name			= "Dynamic Light";
	s_video_dynamic.generic.callback		= f_video_dynamic;
	s_video_dynamic.itemnames				= menu_noyes_names;

    offset += 10;
	s_video_particletype.generic.type		= MTYPE_SPINCONTROL;
	s_video_particletype.generic.x			= 0;
	s_video_particletype.generic.y			= offset;
    ...
    offset += 10;
	s_video_tga_sky.generic.type	       	= MTYPE_SPINCONTROL;
	s_video_tga_sky.generic.x		     	= 0;
	s_video_tga_sky.generic.y		        = offset;
	s_video_tga_sky.generic.name		    = "Use TGA Sky";
	s_video_tga_sky.generic.callback	    = f_video_tga_sky;
	s_video_tga_sky.itemnames			    = menu_noyes_names;

	offset += 10;
	s_video_lightmap_mode.generic.type	    = MTYPE_SPINCONTROL;
	s_video_lightmap_mode.generic.x		    = 0;
	s_video_lightmap_mode.generic.y		    = offset;
	s_video_lightmap_mode.generic.name		= "Lightmap Format";
	s_video_lightmap_mode.generic.callback	= f_video_lightmap_mode;
	s_video_lightmap_mode.itemnames			= menu_lightmap_mode_change;
	
    offset += 20;
	s_video_mipmaps.generic.type		    = MTYPE_SPINCONTROL;
	s_video_mipmaps.generic.x			    = 0;
	s_video_mipmaps.generic.y			    = offset;
	s_video_mipmaps.generic.name		    = "Mipmaps";
	s_video_mipmaps.generic.callback	    = f_video_mipmaps;
	s_video_mipmaps.itemnames			    = menu_noyes_names;

	offset += 10;
	s_video_mipmaps_func.generic.type		= MTYPE_SPINCONTROL;
	s_video_mipmaps_func.generic.x			= 0;
	s_video_mipmaps_func.generic.y		    = offset;
	s_video_mipmaps_func.generic.name		= "Mipmaps Func";
	s_video_mipmaps_func.generic.callback	= f_video_mipmaps_func;
	s_video_mipmaps_func.itemnames			= menu_mipmaps_func_change;
	
    offset += 10;
    extern cvar_t* gu_mipmaps_bias;
    s_video_mipmaps_bias.generic.type	    = MTYPE_SLIDER;
	s_video_mipmaps_bias.generic.x	        = 0;
	s_video_mipmaps_bias.generic.y	        = offset;
	s_video_mipmaps_bias.generic.name	    = "Mipmaps Bias";
	s_video_mipmaps_bias.generic.callback   = f_video_mipmaps_bias;
    s_video_mipmaps_bias.minvalue           = -10.0f;
	s_video_mipmaps_bias.maxvalue           = 0.0f;
	s_video_mipmaps_bias.curvalue           = gu_mipmaps_bias->value;
	...
	s_video_drawfps.generic.y				= offset;
	...
	s_video_drawtime.generic.y				= offset;
	...
	s_video_defaults.generic.y				= offset;
...
#ifdef SOFT_RENDER
    Menu_AddItem(&s_video_menu, (void*)&s_video_stipplealpha);
#elif GU_RENDER	
    Menu_AddItem(&s_video_menu, (void*)&s_video_vsync);
    Menu_AddItem(&s_video_menu, (void*)&s_video_dynamic);
	Menu_AddItem(&s_video_menu, (void*)&s_video_particletype);
	Menu_AddItem(&s_video_menu, (void*)&s_video_tga_sky);
	Menu_AddItem(&s_video_menu, (void*)&s_video_lightmap_mode);
	
	Menu_AddItem(&s_video_menu, (void*)&s_video_mipmaps);
	Menu_AddItem(&s_video_menu, (void*)&s_video_mipmaps_func);
	Menu_AddItem(&s_video_menu, (void*)&s_video_mipmaps_bias);
#endif
