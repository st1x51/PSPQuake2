...
void S_WriteLinearBlastStereo16 (void)
{
	//int		i;
	int		val;
	int mixcount = snd_linear_count;
    int* snd_p_copy = snd_p;
    short* snd_out_copy = snd_out;
    do
    {
        val = *(snd_p_copy++)>>8;
		if (val > 0x7fff)
...
//			Com_Printf ("clear\n");
		...
		{	// copy from the streaming sound source
			...
			for (i=paintedtime ; i<stop ; i++)
			{
				...
				left =  (s_rawsamples[s].left*snd_vol)>>8;
				right = (s_rawsamples[s].right*snd_vol)>>8;
				paintbuffer[i-paintedtime].left = left;// = s_rawsamples[s];
                paintbuffer[i-paintedtime].right = right;// = s_rawsamples[s];			
			}
//		if (i != end)
