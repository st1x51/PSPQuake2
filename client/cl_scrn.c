...
void SCR_Init (void)
{
	...
	scr_graphheight = Cvar_Get ("graphheight", "0", 0);
	scr_graphscale = Cvar_Get ("graphscale", "0", 0);
...
void SCR_DrawFPS(void) {
	...
	for(i = 0; i < sizeof(fpsstring) - 1; i++) {
		re.DrawChar(viddef.width - (7 * 8) + (i * 8), 0, fpsstring[i]);
	}
}
...
void SCR_DrawTime(void) 
{
	...
	for(i = 0; i < sizeof(timestring) - 1; i++) {
		re.DrawChar(viddef.width - (7 * 8) + (i * 8), 8, timestring[i]);
	}
}
