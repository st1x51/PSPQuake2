...
// draw the background
	...
	const char line1[] = "Crow_bar";
    
	char line2[24];
#ifdef GU_RENDER
	sprintf (line2, "QuakeII HW %4.2f", (float)DEFBUILD);
#else
    sprintf (line2, "QuakeII SW %4.2f", (float)DEFBUILD);
#endif

	for (x=0 ; x<strlen(line1) ; x++)
	{
		re.DrawChar (476 - strlen(line1) * 8 + x * 8, viddef.height - 20, 128 + line1[x]);
	}

	for (x=0 ; x<strlen(line2) ; x++)
	{
		re.DrawChar (476 - strlen(line2) * 8 + x * 8, viddef.height - 12, line2[x]);
	}
