...
#ifdef IOCTRL
					FS_Fseek (*file, pak->files[i].filepos, SEEK_SET);
#else
...
#endif
	unsigned		checksum;
	dpackfile_t		*info = (dpackfile_t *)malloc(sizeof(dpackfile_t) * MAX_FILES_IN_PACK);

	if (info == NULL)
	{
		Sys_Error("Couldn't allocate memory for the info section!\n");
	}
...
#ifdef IOCTRL
    FS_Fread (&header, 1, sizeof(header), packhandle);
#else
...
#ifdef IOCTRL
    FS_Fseek (packhandle, header.dirofs, SEEK_SET);
	FS_Fread (info, 1, header.dirlen, packhandle);
#else
...
void FS_SetGamedir (char *dir)
{
	...
	while (fs_searchpaths != fs_base_searchpaths)
	{
		if (fs_searchpaths->pack)
		{
            ...
            {
#ifdef IOCTRL
				FS_Fclose(fs_searchpaths->pack->handle);
#else
